...
function App() {
    ...
    const fetchMessages = async () => {
        try {
            const response = await axios.get('http://localhost:8080/messages/all');
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:8080/messages/${id}`);
            fetchMessages();
        } catch (error) {
    ...
    const handleSearch = async (keyword) => {
        ...
        try {
            const response = await axios.get(`http://localhost:8080/messages/search?keyword=${keyword}`);
